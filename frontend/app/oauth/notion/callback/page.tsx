...
export default function NotionOAuthCallbackPage() {
  ...
  const [message, setMessage] = useState("Notion 인증을 처리 중입니다...");
  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      ...
      try {
        ...
        setMessage("Notion에서 토큰을 가져오는 중...");

        // 서버 사이드 API Route로 토큰 교환 요청
        const tokenResponse = await fetch("/api/oauth/notion/callback", {
          method: "POST",
          headers: {
        ...
        setStatus("success");
        setMessage(result.message || "Notion 계정 연결이 완료되었습니다!");
